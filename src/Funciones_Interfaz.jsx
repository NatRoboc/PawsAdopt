...
function NavbarSuperior(){
    return(
        <header className="grid grid-cols-[auto_auto_auto_auto] items-center gap-6 bg-pink-300 p-4 min-h-[4rem] md:min-h-[5rem] w-full fixed top-0 z-100">
            {/*Logo de PawsAdopt.*/}
            <div className="Logo">
                <Link to="/" className="block"> [LogoPawsAdopt] </Link>
            </div>
                ...
                <ul className="flex flex-row justify-center space-x-3">
                    {
                        menuItems.map((item, index) => {
                            return (
                                <li key={index} className="inline-block px-6 py-2 bg-pink-400 hover:bg-pink-200 rounded-md duration-300 cursor-pointer shadow-sm">
                                    <p>{item.label}</p>
                                </li>
                            )
                        })
...
function ContenedorPrincipal(){
    return(
        <div className="h-full p-4 w-full">
            ...
            <div className="mt-2">Explora adopciones, foros y más.</div>
        </div>
    );
}
                ...
                <ul>
                    {
                        menuCategorias.map((item, index)=> {
                            return(
                                <li key={index} className="block items-center px-6 py-2 text-lg hover:bg-sky-200 rounded duration-300 cursor-pointer shadow-sm">
                                    <p>{item.label}</p>
                                </li>
                            );
                        })
    ...
    return(
        <nav className="grid grid-rows-[9%_auto_auto_auto_auto] p-4 h-full bg-sky-100 w-full">
            ...
            <div className="flex justify-center items-center row-start-1 row-end-2 col-span-full mb-4 px-3 py-2 bg-pink-400 hover:bg-pink-200 rounded-md duration-300 cursor-pointer shadow-sm">
                <Link to="/profile" className="block text-xl">Perfil</Link>
            </div>
                ...
                <ul>
                    {
                        menuUsuario.map((item, index)=> {
                            return(
                                <li key={index} className="block items-center gap-6 px-6 py-2 text-lg hover:bg-sky-200 rounded duration-300 cursor-pointer shadow-sm">
                                    <p>{item.label}</p>
                                </li>
                            );
                        })
                ...
                <ul className="flex justify-center items-center gap-3">
                    {
                        menuFooter.map((item, index)=> {
                            return(
                                <li key={index} className="p-4 px-3 py-2 bg-pink-400 hover:bg-pink-200 rounded-md duration-300 cursor-pointer shadow-sm">
                                    <p>{item.label}</p>
                                </li>
                            )
                        })
